...
public class PacketRegistryImpl implements PacketRegistry {
    ...
    public void registerPacket(Class<? extends Packet> packetClass) {
        ...
        PacketDescription packetDescription = MetaBasedPacketDescription.createPacketDescription(packetClass);
    ...
    public boolean isPacketRegistered(Class<? extends Packet> packetClass) {
        return packetDescriptions.values().stream().anyMatch(packetDescription -> packetDescription.getPacketClass().isAssignableFrom(packetClass));
    }
}
